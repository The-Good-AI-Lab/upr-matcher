        ...
        source_blocks.append(
            {
                ...
                "source_file": str(VALIDATION_ROOT / config.source_pdf),
                "stakeholder_file": str(VALIDATION_ROOT / config.stakeholder_pdf),
                "validation_workbook": str(workbook_path),
        ...
        for row_index, row in enumerate(sheet["rows"], start=1):
            ...
            for target_id, target_text in split_target_recommendations(target_cell):
                ...
                impact_links_by_key[key] = {
                    ...
                    "source_file": str(VALIDATION_ROOT / config.source_pdf),
                    "stakeholder_file": str(VALIDATION_ROOT / config.stakeholder_pdf),
                    "reference_file": str(VALIDATION_ROOT / config.reference_doc),
                    "validation_workbook": str(workbook_path),
                    "source_language": config.source_language,
    ...
    for config in configs:
        ...
        lines.extend(
            [
                ...
                f"    source_pdf: {VALIDATION_ROOT / config.source_pdf}",
                f"    stakeholder_pdf: {VALIDATION_ROOT / config.stakeholder_pdf}",
                f"    reference_doc: {VALIDATION_ROOT / config.reference_doc}",
                f"    validation_workbook: {VALIDATION_ROOT / config.workbook}",
